...
namespace TabTale.PSDK.UnityEditor
{
    ...
    public static class ManifestMod
    {
		...
		private static bool UpdateAndroidManifestFromMod(string androidManifestFile,string modPath) {
			...
			FixUnityPlayerNativeActivity (mainManNode);
		...
		public static void GenerateInitialManifest()
        {
            ...
            if (!File.Exists(outputFile))
            {
				var files = System.IO.Directory.GetFiles (Path.Combine(EditorApplication.applicationPath,".."), ANDROID_MANIFEST_XML, SearchOption.AllDirectories);
				if (files.Length == 0) {
					UnityEngine.Debug.LogError ("Didn't find initial AndroidMAnifest.xml under " + Path.Combine(EditorApplication.applicationPath,".."));
					return;
				}
		...
		private static void UpdateAndroidManifestNodesOrder(XmlNode mainManNode) {
			...
			// making AppsFlyer multiple receiver first (requested by appsFlyer)
			// they will make sure to call all the rest

			XmlNode appsFlyerMultipleReceiverNode = null;
			...
			while (curr != null) {
				...
				if (androidNameValue == "com.appsflyer.MultipleInstallBroadcastReceiver") {
					if (otherReceivers.Count == 0) { 
						...
						appsFlyerMultipleReceiverNode = recievrNode;
					}
				} else {
					otherReceivers.Add (recievrNode);
				}
			}
			// If no appsFlyer MultipleInstallBroadcastReceiver, nothing to do
			if (appsFlyerMultipleReceiverNode != null) {

				// Make MultipleInstallBroadcastReceiver first receiver
				applicationNode.RemoveChild (appsFlyerMultipleReceiverNode);
				foreach (XmlNode n in otherReceivers) {
					applicationNode.RemoveChild (n);
				}
				applicationNode.AppendChild (appsFlyerMultipleReceiverNode);
				foreach (XmlNode n in otherReceivers) {
		...
		private static void FixUnityPlayerNativeActivity(XmlNode mainManNode) {
			...
			if (! ttunityActivityExist) {
				string txt  = 
									"<activity android:name=\"com.tabtale.publishing.ttunity.TTUnityPlayerNativeActivity\" android:label=\"@string/app_name\">"
									+"	<intent-filter>"
