...
//		}
		...
		private static void MoveLaunchImagesFiles(string pathToBuiltProject) {
			...
			try {
				...
				dict.Add("LaunchImage-Portrait@3x.png","Default-Portrait@3x.png");
				dict.Add("LaunchImage-Landscape@3x.png","Default-Landscape@3x.png");
			...
			try {
				...
				foreach(KeyValuePair<string,PBXFileReference> item in  project.fileReferences) {
					if (item.Value.data.ContainsKey("path")) {
						...
						backRefMapping.Add(item.Value.data["path"].ToString(),item.Key.ToString());
					}
				}
				foreach(KeyValuePair<string,PBXBuildFile> item in  project.buildFiles) {
					if (item.Value.data.ContainsKey("fileRef")) {
						...
						if (! backRefMapping.ContainsKey(item.Value.data["fileRef"].ToString())) {
							backRefMapping.Add(item.Value.data["fileRef"].ToString(),item.Key.ToString());
						}
					}
				}
