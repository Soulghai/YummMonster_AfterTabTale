...
public class BubbleField : MonoBehaviour {
	...
	GameObject TryToCreateNewBubble() {
		...
		while ((_cantCreate) && (_cnt < 500)) {
			_pos = new Vector2(transform.position.x + Random.Range(-magnet.radius * 1.15f,Random.value* magnet.radius * 1.15f),
	...
	int ChooseColor() {
		...
		if (level <= 0) {
			...
			if (level <= 3) {//
				if (_ran <= 0.5f) currentColor = DefsGame.BUBBLE_COLOR_ONE; else
				...
				if (level <= 6) {
					if (_ran <= 0.4f) currentColor = DefsGame.BUBBLE_COLOR_ONE; else
						if (_ran <= 0.8f) currentColor = DefsGame.BUBBLE_COLOR_TWO; else
							currentColor = DefsGame.BUBBLE_COLOR_MULTI;
					currentSize = 4 + Mathf.RoundToInt(Random.value * (5f-4f));
				} else
					if (level <= 10) {
						if (_ran <= 0.25f) currentColor = DefsGame.BUBBLE_COLOR_ONE; else
							if (_ran <= 0.50f) currentColor = DefsGame.BUBBLE_COLOR_TWO; else 
								if (_ran <= 0.75f) currentColor = DefsGame.BUBBLE_COLOR_THREE; else 
									currentColor = DefsGame.BUBBLE_COLOR_MULTI;
						currentSize = 3 + Mathf.RoundToInt(Random.value * (5f-4f));
					} else
						if (level <= 13) {
							if ((_ran <= 0.10f)&&(_arrAllLength > _arrTimersLength+_arrHeavyLength)) currentColor = DefsGame.BUBBLE_COLOR_TIMER; else
								if (_ran <= 0.30f) currentColor = DefsGame.BUBBLE_COLOR_ONE; else
									if (_ran <= 0.50f) currentColor = DefsGame.BUBBLE_COLOR_TWO; else 
										if (_ran <= 0.70f) currentColor = DefsGame.BUBBLE_COLOR_THREE; else 
											if (_ran <= 0.90f) currentColor = DefsGame.BUBBLE_COLOR_FOUR; else 
												currentColor = DefsGame.BUBBLE_COLOR_MULTI;
			...
			if (_ranSize < 0.4f)
				...
				currentSize = 5;
							...
							if (level < 17) {
								if ((_ran <= 0.2)&&(_arrAllLength > _arrTimersLength+_arrHeavyLength)) currentColor = DefsGame.BUBBLE_COLOR_TIMER; else
									if ((_ran <= 0.3)&&(_arrAllLength > _arrTimersLength+_arrHeavyLength)) currentColor = DefsGame.BUBBLE_COLOR_HEAVY; else
										if (_ran <= 0.45) currentColor = DefsGame.BUBBLE_COLOR_ONE; else
											if (_ran <= 0.6) currentColor = DefsGame.BUBBLE_COLOR_TWO; else 
												if (_ran <= 0.75) currentColor = DefsGame.BUBBLE_COLOR_THREE; else 
													if (_ran <= 0.9) currentColor = DefsGame.BUBBLE_COLOR_FOUR; else  
														currentColor = DefsGame.BUBBLE_COLOR_MULTI;
								if (_ranSize < 0.4f)
									currentSize = 3;
								...
								if (level < 25) {
									if ((_ran <= 0.15)&&(_arrAllLength > _arrTimersLength+_arrHeavyLength)) currentColor = DefsGame.BUBBLE_COLOR_TIMER; else
										if ((_ran <= 0.3)&&(_arrAllLength > _arrTimersLength+_arrHeavyLength)) currentColor = DefsGame.BUBBLE_COLOR_HEAVY; else
											if (_ran <= 0.45) currentColor = DefsGame.BUBBLE_COLOR_ONE; else
												if (_ran <= 0.6) currentColor = DefsGame.BUBBLE_COLOR_TWO; else 
													if (_ran <= 0.75) currentColor = DefsGame.BUBBLE_COLOR_THREE; else 
			...
			if (_ranSize < 0.6111f)
				...
				currentSize = 5;
									...
									if (level < 40) {
										if ((_ran <= 0.15f)&&(_arrAllLength > _arrTimersLength+_arrHeavyLength)) currentColor = DefsGame.BUBBLE_COLOR_TIMER; else
											if ((_ran <= 0.3f)&&(_arrAllLength > _arrTimersLength+_arrHeavyLength)) currentColor = DefsGame.BUBBLE_COLOR_HEAVY; else
												if (_ran <= 0.45f) currentColor = DefsGame.BUBBLE_COLOR_ONE; else
													if (_ran <= 0.6f) currentColor = DefsGame.BUBBLE_COLOR_TWO; else 
														if (_ran <= 0.75f) currentColor = DefsGame.BUBBLE_COLOR_THREE; else 
															if (_ran <= 0.9f) currentColor = DefsGame.BUBBLE_COLOR_FOUR; else 
																currentColor = DefsGame.BUBBLE_COLOR_MULTI;
										if (_ranSize < 0.35f)
											currentSize = 2;
										if (_ranSize < 0.622f)
	...
	public int GetRandomColor() {
		...
		if (_colorID == DefsGame.BUBBLE_COLOR_MULTI) {
			float _ran = Random.value;
			if (_ran <= 0.25f) return DefsGame.BUBBLE_COLOR_ONE; else
				if (_ran <= 0.5f) return DefsGame.BUBBLE_COLOR_TWO; else
					if (_ran <= 0.75f) return DefsGame.BUBBLE_COLOR_THREE; else
	...
	public AudioClip GetSoundShowClip(int _bubbleID) {
		if ((_bubbleID == DefsGame.BUBBLE_COLOR_ONE)
			...
			float _ran = Random.value;
			if (_ran <= 0.25f) return soundsBubbleShow [0]; else 
				if (_ran <= 0.5f) return soundsBubbleShow [1]; else 
					if (_ran <= 0.75f) return soundsBubbleShow [2]; else 
